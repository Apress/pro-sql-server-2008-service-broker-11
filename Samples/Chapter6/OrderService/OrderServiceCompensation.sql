...
IF EXISTS (SELECT * FROM sys.databases WHERE name = 'Chapter6_OrderServiceCompensation')
BEGIN
	PRINT 'Dropping database ''Chapter6_OrderServiceCompensation''';
	DROP DATABASE Chapter6_OrderServiceCompensation;
END
GO

CREATE DATABASE Chapter6_OrderServiceCompensation
GO

USE Chapter6_OrderServiceCompensation
GO
...
CREATE MESSAGE TYPE [http://ssb.csharp.at/SSB_Book/c06/CreditCardCompensationMessage] VALIDATION = WELL_FORMED_XML
GO
...
(
	...
	[http://ssb.csharp.at/SSB_Book/c06/CreditCardCompensationMessage] SENT BY INITIATOR,
...
CREATE MESSAGE TYPE [http://ssb.csharp.at/SSB_Book/c06/AccountingCompensationMessage] VALIDATION = WELL_FORMED_XML
GO
...
(
	...
	[http://ssb.csharp.at/SSB_Book/c06/AccountingCompensationMessage] SENT BY INITIATOR,
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				...
				-- Check if we must compensate any logic that has already taken place, because the order can not be fulfilled.
				IF (@inventoryResult = 0)
				BEGIN
					-- Variables for the conversation handles and the messages to be sent
					DECLARE @chCreditCardServiceCompensation UNIQUEIDENTIFIER;
					DECLARE @chAccountingServiceCompensation UNIQUEIDENTIFIER;
					DECLARE @msgCreditCardServiceCompensation NVARCHAR(MAX);
					DECLARE @msgAccountingServiceCompensation NVARCHAR(MAX);
					DECLARE @msgCompensation XML;
					DECLARE @chClientServiceCompensation UNIQUEIDENTIFIER;
					DECLARE @orderResponseMessageCompensation NVARCHAR(MAX);

					-- Variables needed to store the information extracted from the OrderRequestMessage
					DECLARE @creditCardHolderCompensation NVARCHAR(256);
					DECLARE @creditCardNumberCompensation NVARCHAR(256);
					DECLARE @validThroughCompensation NVARCHAR(10);
					DECLARE @quantityCompensation INT;
					DECLARE @priceCompensation DECIMAL(18, 2);
					DECLARE @amountCompensation DECIMAL(18, 2);
					DECLARE @customerIDCompensation NVARCHAR(256);

					-- SELECT the original order request message from the OrderQueue - RETENTION makes it possible
					SELECT @msgCompensation = CAST(message_body AS XML) FROM OrderQueue
					WHERE
						conversation_group_id = @conversationGroup AND
						message_type_name = 'http://ssb.csharp.at/SSB_Book/c06/OrderRequestMessage';

					-- Extract the necessary information from the OrderRequestMessage
					SET @creditCardHolderCompensation = @msgCompensation.value('/OrderRequest[1]/CreditCard[1]/Holder[1]', 'NVARCHAR(256)');
					SET @creditCardNumberCompensation = @msgCompensation.value('/OrderRequest[1]/CreditCard[1]/Number[1]', 'NVARCHAR(256)');
					SET @validThroughCompensation = @msgCompensation.value('/OrderRequest[1]/CreditCard[1]/ValidThrough[1]', 'NVARCHAR(10)');
					SET @quantityCompensation = @msgCompensation.value('/OrderRequest[1]/Product[1]/Quantity[1]', 'INT');
					SET @priceCompensation = @msgCompensation.value('/OrderRequest[1]/Product[1]/Price[1]', 'DECIMAL(18, 2)');
					SET @amountCompensation = @quantityCompensation * @priceCompensation;
					SET @customerIDCompensation = @msgCompensation.value('/OrderRequest[1]/Customer[1]/CustomerID[1]', 'NVARCHAR(256)');
					
					-- Begin a new conversation with the CreditCardService on the same conversation group to compensate the previous taken action
					BEGIN DIALOG CONVERSATION @chCreditCardServiceCompensation
						FROM SERVICE [OrderService]
						TO SERVICE 'CreditCardService'
						ON CONTRACT [http://ssb.csharp.at/SSB_Book/c06/CreditCardContract]
						WITH RELATED_CONVERSATION = @ch, ENCRYPTION = OFF;

					-- Send a CreditCardCompensationMessage to the CreditCardService
					SET @msgCreditCardServiceCompensation = 
						'<CreditCardCompensation>
							<Holder>' + @creditCardHolderCompensation  + '</Holder>
							<Number>' + @creditCardNumberCompensation + '</Number>
							<ValidThrough>' + @validThroughCompensation + '</ValidThrough>
							<Amount>' + CAST(@amountCompensation AS NVARCHAR(10)) + '</Amount>
						</CreditCardCompensation>';

					SEND ON CONVERSATION @chCreditCardServiceCompensation MESSAGE TYPE 
						[http://ssb.csharp.at/SSB_Book/c06/CreditCardCompensationMessage] 
						(@msgCreditCardServiceCompensation);

					-- Begin a new conversation with the AccountingService on the same conversation group to compensate the previous taken action
					BEGIN DIALOG CONVERSATION @chAccountingServiceCompensation
						FROM SERVICE [OrderService]
						TO SERVICE 'AccountingService'
						ON CONTRACT [http://ssb.csharp.at/SSB_Book/c06/AccountingContract]
						WITH RELATED_CONVERSATION = @ch, ENCRYPTION = OFF;

					-- Send a AccountingServiceCompensationMessage to the AccountingService
					SET @msgAccountingServiceCompensation = 
						'<AccountingCompensation>
							<CustomerID>' + @customerIDCompensation + '</CustomerID>
							<Amount>' + CAST(@amountCompensation AS NVARCHAR(10)) + '</Amount>
						</AccountingCompensation>';

					SEND ON CONVERSATION @chAccountingServiceCompensation MESSAGE TYPE 
						[http://ssb.csharp.at/SSB_Book/c06/AccountingCompensationMessage] 
						(@msgAccountingServiceCompensation);

					-- End the conversation with the ClientService, because the order request cannot be fulfilled.
					-- Get the conversation handle, that is needed to send a response message back to the ClientService.
					SELECT @chClientServiceCompensation = conversation_handle FROM sys.conversation_endpoints
					WHERE 
						conversation_group_id = @conversationGroup AND
						far_service = 'ClientService';

					-- Send the response message back to the ClientService
					SET @orderResponseMessageCompensation = '<OrderResponse>0</OrderResponse>';
					SEND ON CONVERSATION @chClientServiceCompensation MESSAGE TYPE 
						[http://ssb.csharp.at/SSB_Book/c06/OrderResponseMessage] 
						(@orderResponseMessageCompensation);

					-- End the conversation with the ClientService
					END CONVERSATION @chClientServiceCompensation;
				END
				ELSE
				BEGIN
...
AS
	...
	BEGIN
		...
		IF (@messagetypename = 'http://ssb.csharp.at/SSB_Book/c06/CreditCardCompensationMessage')
		BEGIN
			-- Compensate the previous transaction through a DELETE
			DELETE FROM CreditCardTransactions
			WHERE
				CreditCardHolder = @messagebody.value('/CreditCardCompensation[1]/Holder[1]', 'NVARCHAR(256)') AND
				CreditCardNumber = @messagebody.value('/CreditCardCompensation[1]/Number[1]', 'NVARCHAR(50)') AND
				ValidThrough = @messagebody.value('/CreditCardCompensation[1]/ValidThrough[1]', 'NVARCHAR(10)') AND
				Amount = @messagebody.value('/CreditCardCompensation[1]/Amount[1]', 'DECIMAL(18, 2)');
...
AS
	...
	BEGIN
		...
		IF (@messagetypename = 'http://ssb.csharp.at/SSB_Book/c06/AccountingCompensationMessage')
		BEGIN
			-- Compensate the previous transaction through a DELETE
			DELETE FROM AccountingRecordings 
			WHERE
				CustomerID = @messagebody.value('/AccountingCompensation[1]/CustomerID[1]', 'NVARCHAR(10)') AND
				Amount = @messagebody.value('/AccountingCompensation[1]/Amount[1]', 'DECIMAL(18, 2)');
